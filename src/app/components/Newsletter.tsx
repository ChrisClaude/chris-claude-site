...
const Newsletter = ({ compact = false }: {compact?: boolean}) => {
  ...
  return (
    <section className={`w-full bg-slate-800 relative ${compact ? 'rounded-lg pt-14 pb-20 px-10': 'py-28 px-5'}`}>
      ...
      <div className={`w-full flex flex-col gap-4 md:justify-center ${compact? 'md:flex-col' : 'md:flex-row md:w-2/3'} ${isSubscribed ? ' hidden' : ''}`}>
        <div className={`${compact? 'md:w-full' : 'md:w-1/3'}`}>
          <input
            ...
            <p className={`p-1 bg-red-50 text-center text-sm text-red-600 ${isError? '' : ' hidden'}`}>
