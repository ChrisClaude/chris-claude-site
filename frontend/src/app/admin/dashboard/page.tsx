...
const AdminDashboardPage = () => {
  ...
  const { data: myPostsData } = useQuery<{ myPosts: ConnectionDto<PostDto> }>(GET_MY_POSTS, {
    variables: { first: 5 },
    fetchPolicy: 'cache-and-network',
  });

  const { data: allPostsData } = useQuery<{ allPostsAdmin: ConnectionDto<PostDto> }>(
    GET_ALL_POSTS_ADMIN,
    { variables: { first: 1 }, fetchPolicy: 'cache-and-network' },
  );

  const { data: usersData } = useQuery<{ users: ConnectionDto<UserDto> }>(GET_USERS, {
    variables: { first: 1 },
    fetchPolicy: 'cache-and-network',
  });
  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
        <StatCard label="My Articles" value={myPostsTotal} href="/admin/my-articles" />
        <StatCard label="Total Articles" value={allPostsTotal} href="/admin/all-articles" />
